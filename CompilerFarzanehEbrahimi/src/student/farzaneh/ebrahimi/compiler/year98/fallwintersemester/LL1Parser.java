...
{
	public static boolean parse(LinkedList<String> input, Map<Integer, String> parseTable)
	{		
		// prepare stack
...
//		stack.push(parseTable.getStart());
		...
		while (! (stack.isEmpty() && input.isEmpty()) )
		{
			...
			// if terminal look up 
			if(true)//topRule.isTerminal())
			{
				if(true)//topRule.symbol.equals(inputToken)) // matching input and stack tokens
				{
				...
				else if(!false)//topRule.equals(EpsilonRule.getEpsilonRule())) // the tokens are mismatched
...
//							" does not match with stack token " + topRule.getSymbol() + ".\n " + 
			...
			else
			{
				...
				LinkedList<GrammarRule> expandedRule = null;//parseTable.get( (NonterminalRule) topRule, inputToken);
